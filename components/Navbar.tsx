			...
			<motion.nav
				...
				<motion.div
					...
					<ul className='hidden justify-center items-center gap-6 sm:flex flex-row'>
						{NAV_LINKS.map((link) => (
							<li className='bg-slate-300/60 hover:bg-blue-300/60 hover:scale-105 px-2 py-0.5 rounded-md text-slate-900 font-medium text-sm md:text-md'>
								<Link
									href={link.href}
									key={link.key}>
									{link.label}
					...
					<div className='flexCenter gap-4 relative right-[-10]'>
						...
						<Link href='/'>
							<button className='bg-slate-200/60 flex justify-center text-center text-md p-2 rounded-full items-center'>
								<FaUser />
							</button>
						</Link>
					</div>
