			...
			<motion.nav
				...
				<motion.div
					...
					<ul className='hidden justify-center items-center gap-6 sm:flex flex-row'>
						{NAV_LINKS.map((link) => (
							<li className='bg-slate-300/60 hover:bg-orange-500/60 hover:scale-105 px-2 py-0.5 rounded-md text-slate-900 font-medium text-sm md:text-md transition-all duration-300'>
								<Link
									href={link.href}
									key={link.key}>
									{link.label}
